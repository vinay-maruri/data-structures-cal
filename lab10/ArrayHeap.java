...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private boolean compare(int x) {
        return (min(x, parentIndex(x)) == parentIndex(x));
    }
    /**
    ...
    private void sink(int index) {
        ...
        while (2*index <= size) {
            int x = 2 * index;
            if (x < size && greatcompare(x, x+1)) {
                x += 1;
            }
            if (!greatcompare(index, x)) {
                break;
            }
            swap(index, x);
            index = x;
    ...
    private boolean greatcompare(int y, int z){
        return min(y, z) == z;
    }
    /**
    ...
    public void insert(T item, double priority) {
        ...
        Node nodeAdd = new Node(item, priority);
        size += 1;
        contents[size] = nodeAdd;
        swim(size);
    }
